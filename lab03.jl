...
# ╔═╡ 65e75ec9-c6c0-4116-8fc6-1c757ed0f50a
...
size(train_x_flatten)[1], size(train_y), size(test_x_flatten), size(test_y)
end
...
function compute_average_intensity(x, y)
    ...
    #TODO compute average intensity for each label
	for i in 0:9
		mean_i=mean(x[y.==i,:],dims=2)[:]
		mean_[i+1]=mean(mean_i)
	end
    return mean_
end
...
begin
	...
	intensity=[mean(train_x_flatten[i,:]) for i in 1:size(train_x_flatten,1)]
	size(intensity)
end

# ╔═╡ 99a45759-4ae3-44ad-b296-b181248ddca3
intensity
...
Phương pháp chuẩn hóa Z-Score đã xuất hiện từ những năm đầu thế kỷ 19, khi Gauss ra đời giới thiệu về phân phối chuẩn. Ngày nay nó được áp dụng rộng rãi trong khoa học dữ liệu nhờ sự gia tăng của các thuật toán máy học và khai thác tri thức.

Z-score normalization (hay Standardization) là một phương pháp chuẩn hóa dữ liệu trong đó giá trị của một biến được chuyển đổi để có giá trị trung bình bằng 0 và độ lệch chuẩn bằng 1. Với phương pháp này, phạm vi của dữ liệu sẽ không thay đổi nhưng làm cho đặc trưng của dữ liệu sẽ thay đổi theo phân bố chuẩn (normal distribution), giúp cho việc khai thác dữ liệu, phân tích, so sánh trở nên thuận tiện và hiệu quả hơn.

###### Công thức tính Z-score của giá trị x là:

Z = (x – mean) / standard_deviation

###### Trong đó:

	- mean là giá trị trung bình của biến
	- standard_deviation là độ lệch chuẩn của biến
##### TODO: Why should we normalize data?

- Tạo ra sự cân bằng giữa các thuộc tính của dữ liệu: khi dữ liệu chưa được chuẩn hóa, các giá trị giữa các thuộc tính dễ có phạm vi hoặc đơn vị rất khác nhau. Bằng cách áp dụng phương pháp Z-score chuẩn hóa, ta có thể đưa các giá trị của các thuộc tính thành cùng một miền giá trị, tạo ra một sự cân bằng giữa các thuộc tính trong dữ liệu.
- Giảm thiểu ảnh hưởng của nhiễu (noise) và outliers: Các giá trị nhiễu và outliers có thể làm sai lệch dữ liệu và ảnh hưởng đến việc phân tích hoặc xử lý dữ liệu. Phương pháp Z-score chuẩn hóa giúp giảm thiểu ảnh hưởng này bằng cách loại bỏ sự ảnh hưởng của mean trên từng điểm dữ liệu riêng lẻ, làm giảm đáng kể sự thay đổi của các giá trị trong tập dữ liệu.
- Giúp cho các thuật toán hoạt động hiệu quả hơn: nhiều thuật toán trong Máy học giả định rằng dữ liệu đầu vào có phân phối Gauss. Chính vì vậy, khi chuẩn hóa dữ liệu về dạng chuẩn phân phối Gauss với giá trị trung bình bằng 0 và phương sai bằng 1 giúp các thuật toán như linear regression, logistic regression được cải thiện.
...
Accuracy score trên tập train và tập test đều cao và gần bằng nhau (xấp xỉ 0.92) , điều này có thể cho thấy mô hình Logistic Regression dùng để phân loại trên tập dữ liệu này không bị overfitting hoặc underfitting. Hay mô hình phân loại này hoạt động hiệu quả và có khả năng dự đoán chính xác trên các tập dữ liệu mới.

Nhận thấy trên cả tập train và tập test, có precision và accuracy cao trong khi recall thấp, có thể đây là do mô hình có xu hướng dự đoán sai những trường hợp Negative (không có giá trị) với số lượng lớn, dẫn đến Recall thấp. Đồng thời, mô hình lại có xu hướng dự đoán chính xác các trường hợp Positive (có giá trị), dẫn đến Precision và Accuracy cao.

Để khắc phục vấn đề này, ta cần phải tập trung vào việc tăng Recall bằng cách cân bằng lại số lượng mẫu Positive và Negative trong dữ liệu huấn luyện hoặc áp dụng các kỹ thuật tham số hóa mô hình để tối ưu lại quá trình dự đoán.
"""
